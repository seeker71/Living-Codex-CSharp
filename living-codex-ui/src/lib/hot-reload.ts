...
export function useHotReload() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const response = await fetch('http://localhost:5002/hot-reload/status');
      if (response.ok) {
  ...
  const startWatching = useCallback(async (config?: {
    ...
    try {
      const response = await fetch('http://localhost:5002/hot-reload/start', {
        method: 'POST',
  ...
  const stopWatching = useCallback(async () => {
    try {
      const response = await fetch('http://localhost:5002/hot-reload/stop', {
        method: 'POST'
      });
  ...
  const regenerateComponent = useCallback(async (componentId: string, options?: {
    ...
    try {
      const response = await fetch('http://localhost:5002/hot-reload/regenerate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          componentId,
  ...
  const hotSwapComponent = useCallback(async (componentPath: string, newCode: string) => {
    try {
      const response = await fetch('http://localhost:5002/hot-reload/swap', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          componentPath,
  ...
  const getHistory = useCallback(async (limit: number = 50) => {
    try {
      const response = await fetch(`http://localhost:5002/hot-reload/history?limit=${limit}`);
      if (response.ok) {
        const data = await response.json();
        if (data.success) {
...
export function useHotReloadNotifications() {
  ...
  useEffect(() => {
    ...
    const pollForEvents = async () => {
      try {
        const response = await fetch('http://localhost:5002/hot-reload/history?limit=5');
        if (response.ok) {
          const data = await response.json();
